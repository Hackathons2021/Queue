...
class EnterCode : AppCompatActivity() {
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        val nameText:EditText = findViewById(R.id.editName)

        submit.setOnClickListener {
            if (TextUtils.isEmpty(nameText.text.toString()) || TextUtils.isEmpty(nameText.text.toString())) {
                Toast.makeText(
                    this@EnterCode,
                    "Enter shop ID and a Name",
                    Toast.LENGTH_SHORT
                ).show()
                ...
                when {
                    (codeEntry.text.toString().trim { it <= ' ' }.length != 6) -> {
                        ...
                        myRefb.child("codeList").child(code).get().addOnSuccessListener {
                            ...
                            Log.d("2", businessId)
                            myRefb.child("userCustomers").child(userId).child("username").setValue(nameText.text.toString())
